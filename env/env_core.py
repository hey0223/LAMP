...
class economic_society:
    ...
    def step(self, action_dict):
        ...
        if np.sum(choose_consumption) + Gt > self.GDP:
            logging.info(f"{np.sum(choose_consumption) + Gt} > {self.GDP}")
            logging.info("consumption is too high")
            self.done = True
        ...
        self.done = self.done or bool(self.wealth_gini > 0.9 or self.income_gini>0.9 or math.isnan(self.government_reward *self.wealth_gini * self.income_gini) or
          ...
          # ② 判断各项终止条件，并记录详细信息
        if self.wealth_gini > 0.9:
            logging.info("Termination triggered: wealth_gini > 0.9: {:.3f} > 0.9".format(self.wealth_gini))
        if self.income_gini > 0.9:
            logging.info("Termination triggered: income_gini > 0.9: {:.3f} > 0.9".format(self.income_gini))
        if math.isnan(self.government_reward * self.wealth_gini * self.income_gini):
            logging.info("Termination triggered: product of government_reward, wealth_gini, income_gini is NaN")
        if math.isnan(np.mean(self.households_reward)):
            logging.info("Termination triggered: mean of households_reward is NaN")
        if np.min(self.households.at_next) < 0:
            logging.info("Termination triggered: min(households.at_next) < 0: {:.3f} < 0".format(np.min(self.households.at_next)))
        if self.Kt_next < 0:
            logging.info("Termination triggered: Kt_next < 0: {:.3f} < 0".format(self.Kt_next))

        if math.isnan(self.government_reward * self.wealth_gini * self.income_gini) or math.isnan(np.mean(self.households_reward)):
            logging.info("Termination triggered: product of government_reward, wealth_gini, income_gini is NaN or mean of households_reward is NaN")
            self.done = True
        ...
        current_step = self.step_cnt + 1
        if current_step % 20 == 0 or self.done:
            logging.info(
                f"[Env Step {current_step}] GDP={self.per_household_gdp:.6f}, income_gini={self.income_gini:.6f}, "
                f"wealth_gini={self.wealth_gini:.6f}, wage={self.WageRate:.6f}, Bt={self.Bt:.6f}, Kt={self.Kt:.6f}, "
                f"tau={self.government.tau:.6f}, xi={self.government.xi:.6f}, tau_a={self.government.tau_a:.6f}, "
                f"xi_a={self.government.xi_a:.6f}, Gt_prob={self.Gt_prob:.6f}, gov_reward={self.government_reward:.6f}, "
                f"house_reward_mean={float(np.mean(self.households_reward)):.6f}"
            )
