...
class Household(BaseEntity):  
    ...
    @staticmethod
    def _ordered_agent_ids(n_households: int):
        return [f"Household{i+1}" for i in range(n_households)]

    @staticmethod
    def _normalize_agent_score_map(raw_map, ordered_agent_ids, default_value, cast_type=int):
        """
        Normalize an LLM-returned dict keyed by agent_id.
        Ensures all agent_ids exist and values are castable.
        """
        ...
        for agent_id in ordered_agent_ids:
            value = raw_map.get(agent_id, default_value)
            try:
                normalized[agent_id] = cast_type(value)
            except Exception:
                return None
        return normalized
    ...
    @staticmethod
    def _request_retry_budget(max_retries: int) -> int:
        return max(1, min(2, int(max_retries)))

    @staticmethod
    def _default_analysis_result():
        return {
            "analysis": "LLM output unavailable; using a neutral fallback assessment.",
            "economic_status": 1,
            "reasoning": "Maintain a cautious, balanced plan until clearer information arrives.",
            "statements": [
                "I will stay cautious and avoid abrupt economic moves until conditions are clearer.",
                "I am balancing labor and savings carefully while the outlook remains uncertain.",
                "Stability matters most right now, so I will adjust gradually instead of reacting aggressively."
            ]
        }

    @staticmethod
    def _default_reasoning_result():
        return {
            "economic_status": 1,
            "reasoning": "The signal is uncertain, so keep labor and savings choices moderate until stronger evidence appears."
        }

    @staticmethod
    def _default_evaluation_result(current_agent_id, ordered_agent_ids):
        return {
            ...
            "reflection_text": "LLM evaluation unavailable; using neutral fallback beliefs and trust levels."
        }

    @staticmethod
    async def analyze_and_communicate(
        long_term_news, similar_experience, private_observation,
        temperature=0.7, max_retries=4
    ):
        """
        ...
        **Tasks:**
        ...
        4. Generate exactly 3 public statements in "statements" that are strategically distinct and mutually contrastive.
           • The 3 statements must differ in policy stance, implied action, and rhetorical framing.
           • Do not produce paraphrases, near-duplicates, or three generic versions of the same macro opinion.
           • Each statement must be a single public-facing sentence, concrete, and under 35 words.
        ...
        **Output Format:**
        ...
        failed_attempts = []
        request_max_retries = Household._request_retry_budget(max_retries)
        
        for attempt in range(max_retries):
            llm_output = await async_llm_call(
                prompt,
        ...
        return Household._default_analysis_result()

    @staticmethod
    async def derive_reasoning_text(
        short_term_news, recent_long_term_news,
  ...
  • Personal wealth: {private_observation[1]:.4f}"""
        ...
        **Tasks:**
        1. Provide a detailed analysis of current economic conditions, provide guidance for choosing savings rate and working hours.
        ...
        **Output Format:**
        ...
        return Household._default_reasoning_result()
    ...
    async def evaluate_agents_statements(
        ...
        ordered_agent_ids = Household._ordered_agent_ids(self.n_households)

        if isinstance(other_agents_statements, list) and len(other_agents_statements) > 0:
            formatted_lines = []
            for item in other_agents_statements:
                if not isinstance(item, dict):
                    continue
                speaker_id = item.get("agent_id", "UnknownAgent")
                statement = item.get("statement", "")
                if statement:
                    formatted_lines.append(f'- {speaker_id}: "{statement}"')
            other_statements_formatted = "\n".join(formatted_lines) if formatted_lines else "No other statements available."
        else:
        ...
        All household IDs in this environment:
        {ordered_agent_ids}

        Your final goal is to improve the self-utility of the current family, where increased labor time reduces utility and increased consumption improves utility, under the Bewley–Aiyagari model.

        Tasks:
        1. For EVERY household ID in the provided household list, infer a wealth level and store it in "wealth_guesses_by_agent".
        - Use keys exactly equal to the household IDs.
        - Value meanings: 0=Low, 1=Medium, 2=High.
        2. For EVERY household ID in the provided household list, assign a trust level and store it in "trust_levels_by_agent".
        - Use keys exactly equal to the household IDs.
        - Value range: 0 to 10.
        - The current agent ({current_agent_id}) should trust itself highly.
        3. Provide a brief reflection in "reflection_text", focusing on how these statements may affect future economic decisions.
        ...
        Important:
        - Include ALL household IDs from the provided list.
        - Do not omit the current agent.
        - Do not use arrays for wealth or trust output.
        - Use only the exact household IDs as keys.
        """
        required_keys = {"wealth_guesses_by_agent", "trust_levels_by_agent", "reflection_text"}
        request_max_retries = Household._request_retry_budget(max_retries)
        failed_attempts = []
        for attempt in range(max_retries):
            ...
            if set(result.keys()) != required_keys:
                failure_info["error"] = f"Validation failed: missing_keys={required_keys - set(result.keys())}, extra_keys={set(result.keys()) - required_keys}"
                failed_attempts.append(failure_info)
                continue

            wealth_map = Household._normalize_agent_score_map(
                result.get("wealth_guesses_by_agent"),
                ordered_agent_ids,
                default_value=1,
                cast_type=int
            )
            trust_map = Household._normalize_agent_score_map(
                result.get("trust_levels_by_agent"),
                ordered_agent_ids,
                default_value=5,
                cast_type=int
            )

            if wealth_map is None or trust_map is None:
                failure_info["error"] = "Validation failed: wealth_map or trust_map normalization returned None"
                failed_attempts.append(failure_info)
                continue

            # clamp values
            wealth_map = {k: min(2, max(0, int(v))) for k, v in wealth_map.items()}
            trust_map = {k: min(10, max(0, int(v))) for k, v in trust_map.items()}

            # self-trust floor
            trust_map[current_agent_id] = max(trust_map[current_agent_id], 9)

            return {
                "wealth_guesses_by_agent": wealth_map,
                "trust_levels_by_agent": trust_map,
                "reflection_text": result.get("reflection_text", "")
            }

        logging.error("Exceeded max retries for evaluate_agents_statements")
        return Household._default_evaluation_result(current_agent_id, ordered_agent_ids)
